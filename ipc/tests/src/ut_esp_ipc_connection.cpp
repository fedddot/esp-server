...
#include "esp_ipc_connection.hpp"
...
int main(void) {
	// GIVEN
	const EspIpcConnection::Baud baud(EspIpcConnection::Baud::B9600);
	...
	// WHEN
	EspIpcConnection *instance_ptr(nullptr);

	// THEN
	try {
		EspIpcConnection instance(baud, serializer, matcher, extractor);
		instance.subscribe(
			"test_subsc",
